...
public class WebsocketClient
{
    ...
    {
        Instance = this;
        /*using (WebSocket = new WebSocket("wss://gateway.discord.gg/"))
        {
            WebSocket.OnOpen += (sender, e) => WebSocket_OnConnect();
            WebSocket.OnClose += (sender, e) => WebSocket_OnDisconnected(e.Code, e.Reason);
            //WebSocket.OnMessage += (sender, e) => WebSocket_OnMessageReceived(e.Data);

            WebSocket.Connect();
        }*/
    }

    public static bool IsConnected() => Instance.Logged;

    private void WebSocket_OnConnect()
    {
        Console.WriteLine($"[Discord] Connection with the gateway has been initiated.");
        //_ = Login();
    }

    private void WebSocket_OnDisconnected(int status, string reason)
    {
        Console.WriteLine($"[Discord] Connection with the gateway has been closed ({status} | {reason}).");
        Logged = false;
    }

    private void WebSocket_OnMessageReceived(string message)
    {
        Console.WriteLine($"[Discord] New message received {message}");
    ...
    /*public async Task Login()
    {
        ...
        WebSocket.SendAsync(JsonSerializer.Serialize(payload), null);
    }*/
}
